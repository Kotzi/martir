...
public class BackgroundLoop : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        ScreenBounds = MainCamera.ScreenToWorldPoint(new Vector3(Screen.width, Screen.height, 10f));
    ...
    void LoadChildObjects(GameObject obj)
    {
        ...
        for (int i = -childsNeededY; i <= childsNeededY; i++)
        {
            ...
            for (int j = -childsNeededX; j <= childsNeededX; j++)
            {
                ...
                c.transform.position = new Vector3(objectWidth * j, 0, 10f);
                c.name = obj.name + i + j;
            }
            clone.transform.position = new Vector3(0, objectHeight * i, 10f);
    ...
    void RepositionChildObjects(GameObject obj) 
    {
        ...
        if(needsAdjustX != 0) 
        {
            for (int i = 0; i < obj.transform.childCount; i++)
            {
                ...
                if(needsAdjustX > 0)
                {
                    ...
                    firstChild.transform.position = new Vector3(lastChild.transform.position.x + halfObjectWidth * 2, firstChild.transform.position.y, 10f);
                }
                else
                {
                    lastChild.transform.SetAsFirstSibling();
                    lastChild.transform.position = new Vector3(firstChild.transform.position.x - halfObjectWidth * 2, lastChild.transform.position.y, 10f);
                }
            }
        }
        ...
        if(needsAdjustY != 0) 
        {
            if(needsAdjustY > 0)
            {
                bottomRoot.transform.position = new Vector3(bottomRoot.transform.position.x, topRoot.transform.position.y + halfObjectHeight * 2, 10f);
                bottomRoot.SetAsLastSibling();
            }
            else
            {
                topRoot.transform.position = new Vector3(topRoot.transform.position.x, bottomRoot.transform.position.y - halfObjectHeight * 2, 10f);
